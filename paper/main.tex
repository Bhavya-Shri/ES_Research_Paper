...
The same matched-LUFS pairs were re-scored with an IEC~61672 C-weighted digital proxy $L_{Ceq}$ (same RMS-plus-offset helper as $L_{Aeq}$; not ear-level SPL). The hypothesis, written before looking, was that the bass-first system (S1) should look better on $L_{Ceq}$ than on $L_{Aeq}$, because C still counts the bass that S1 removes, and that a frozen mid-cut config with no shelf (S2) should do the opposite. C at 40\,Hz is about $-2$\,dB; A is about $-35$\,dB. On $n=20$, S1 mean $\Delta L_{Aeq}=+1.17$\,dB but mean $\Delta L_{Ceq}=-0.58$\,dB (bootstrap 95\% CI $[-0.94,\,-0.26]$; two-sided Wilcoxon $p=1.0\times 10^{-4}$). S2 mean $\Delta L_{Aeq}=-0.27$\,dB but mean $\Delta L_{Ceq}=+0.06$\,dB (CI $[0.02,\,0.12]$; $p=0.007$). Both sign predictions hold. Frequency-selective limiting is not uniformly better or worse than flat gain; it is better for the meter whose weight is large where the processor cuts. These remain digital-domain proxies.
